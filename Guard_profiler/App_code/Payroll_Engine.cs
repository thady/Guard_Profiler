...
namespace Guard_profiler.App_code
{
	internal static class Payroll_Engine
	{
		...
		public static DataTable Return_guard_shift_types(string myQuery, string branch_name,string guard_number, DateTime deploy_start_date, DateTime deploy_end_date)
		{
        ...
        public static DataTable select_guard_payroll_summary(string myQuery, int deploy_period_id, string station_name)
        {
            DataTable dt = new DataTable();
            try
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["sg_conn_str"].ToString()))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_guard_payroll_report_queries", conn))
                        {
                            ...
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.Add("@station_name", SqlDbType.NVarChar, 50);
                            cmd.Parameters["@station_name"].Value = station_name;
